...
self.addEventListener("activate",function(e){e.waitUntil(caches.keys().then(function(ks){return Promise.all(ks.map(function(k){if(k!==C&&k.indexOf("shuppin-")===0)return caches.delete(k)}))}).then(function(){return self.clients.claim()}))});
self.addEventListener("fetch",function(e){
  ...
  // HTMLは常にネットワーク優先。オフラインのときだけキャッシュを使う
  e.respondWith(
